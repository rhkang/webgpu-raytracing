...
fn main(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
    ...
    mySphere.center = sphere.center;
    mySphere.radius = sphere.radius;

    var myRay: Ray;
    myRay.direction = normalize(forwards + horizontal_coefficient * right + vertical_coefficient * up);
    myRay.origin = vec3<f32>(0.0, 0.0, 0.0);

    var pixel_color : vec3<f32> = vec3<f32>(0.1, 0.2, 0.3);

    var light : vec3<f32> = normalize(vec3<f32>(0.3, 0.5, 0.3));

    if (hit(myRay, mySphere)) {
        var amount = calc(myRay, mySphere, light);
        pixel_color = vec3<f32>(0.5 * amount, 1.0 * amount, 0.75 * amount);
    }
...
fn calc(ray: Ray, sphere: Sphere, light: vec3<f32>) -> f32 {
    let a: f32 = dot(ray.direction, ray.direction);
    let b: f32 = 2.0 * dot(ray.direction, ray.origin - sphere.center);
    let c: f32 = dot(ray.origin - sphere.center, ray.origin - sphere.center) - sphere.radius * sphere.radius;

    let discriminant: f32 = b * b - 4.0 * a * c;
    let plus = sqrt(discriminant / 4.0);
    let t = - b / 2.0 - plus;

    let d = ray.direction * t;

    var normal: vec3<f32> = normalize(sphere.center - d);
    var result = dot(normal, light);
